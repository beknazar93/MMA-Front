...
//     switch (msg.type) {
//       case 'image':
//         return msg.media ? <img src={msg.media} alt="Фото" /> : 'Фото';
//       case 'video':
//         return msg.media ? <video controls><source src={msg.media} type="video/mp4" /></video> : 'Видео';
//       case 'audio':
//       case 'ptt':
//         return msg.media ? <audio controls><source src={msg.media} type="audio/ogg" /></audio> : 'Голосовое сообщение';
//       case 'document':
//         return msg.media ? <a href={msg.media} download>Скачать документ</a> : 'Документ';
//       case 'call_log':
...
//               }}
//             >
//               <img src={chat.profilePic || 'default-avatar.png'} alt="Аватар" className="avatar" />
  ...
  return (
    <div className="whatsapp-chat">
      <div className="sidebar">
        ...
        <ul>
          {chats.map((chat) => (
            <li key={chat.id} className={currentChat === chat.id ? 'active' : ''} onClick={() => { setCurrentChat(chat.id); fetchMessages(chat.id); }}>
              <img src={chat.profilePic || 'default-avatar.png'} alt="Аватар" className="avatar" />
              <span className="chat-name">{chat.name || formatPhoneNumber(chat.id)}</span>
